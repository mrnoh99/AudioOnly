...
## 여러 기기에서 같은 폴더 쓰기 (동기화)

iPhone · iPad · Mac이 **같은 iCloud Drive 폴더**를 저장 위치로 쓰면(iOS: 설정 → 저장 위치, Mac: 설정 → 저장 위치):

- 한 기기에서 **추가 · 이름 변경 · 삭제**한 파일이 iCloud 동기화로 다른 기기의 보관함에도 자동으로 반영됩니다.
  폴더 변경 알림을 받아 곧바로, 그리고 앱이 켜져 있는 동안 20초마다 · 앱으로 돌아올 때마다 다시 확인합니다.
- 재생 대기열에 있던 곡의 이름이 다른 기기에서 바뀌면 대기열도 새 이름으로 바뀌고, 재생 중인 곡은 끊기지 않습니다.
- **iCloud 새 파일 자동 받기 (Wi-Fi)**(기본 켜짐): 다른 기기에서 올라온 새 파일을 Wi-Fi에서 미리 받아 두어 바로 재생할 수 있습니다.
- 설정의 **지금 동기화** 버튼으로 바로 다시 확인하고 받을 수 있습니다.
